...
def init_dwpose_detector(device):
    ...
    det_config = os.path.join(script_directory() ,"src","configs","yolox_l_8xb8-300e_coco.py")
    det_ckpt = os.path.join(script_directory() ,"ckpts","yolox_l_8x8_300e_coco_20211126_140236-d3bd2b23.pth")  
    pose_config = os.path.join(script_directory() ,"src","configs","dwpose-l_384x288.py")
    pose_ckpt = os.path.join(script_directory() ,"ckpts","dw-ll_ucoco_384.pth")
...
def inference_pose_by_image(pil_image, image_size=(1024, 1024)):
    device = torch.device(f"cuda:{0}")
    model = init_dwpose_detector(device=device)
